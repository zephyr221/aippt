...
def _check_layout_rhythm(slides: list[dict[str, Any]], suggestions: list[str]) -> None:
    ...
    if len(content_layouts) >= 4 and len(set(content_layouts)) <= 1:
        suggestions.append("版式节奏偏单一，建议加入 metric_strip、milestone_timeline、media_explain、concept_diagram 或 fact cards。")
    rhythms = [
    ...
    for idx in range(0, max(0, len(rhythms) - 2)):
        if rhythms[idx] and len({rhythms[idx], rhythms[idx + 1], rhythms[idx + 2]}) == 1:
            suggestions.append(
                f"第 {idx + 1}-{idx + 3} 张视觉节奏连续重复，可换成 metric_strip、milestone_timeline、project_showcase、media_explain、concept_diagram 或 table。"
            )
            break

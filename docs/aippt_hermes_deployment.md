...
## Xiaomi MiMo Key 的使用边界

当前 MiMo key 的套餐说明写明：仅限在兼容的 AI 编程和智能体工具中交互式使用，
不可用于自动化脚本或应用后端。AIPPT 的目标形态是用户显式发起、可观察、可
迭代的 agent PPT 工作台，不是无人值守批量生成 PPT 的自动化后端。基于这个理解：

- 可以用于服务器上的人工研发、调试、dogfood，以及用户显式点击触发的
  Hermes/MiMo 深度规划、改写大纲、AI 审稿和局部修复。
- 不应把该 key 写入仓库、浏览器代码、API 响应或前端环境变量。
- 不应把该 key 用于隐藏后台批量生成、无人值守定时任务或公开大规模自助服务。
- 每次 Hermes/MiMo agent 运行都应记录 provider、model、prompt version 和输入输出摘要，
  让用户知道这次是快速 deterministic 路径还是 agent 深度路径。
- 如果未来产品形态变成公开高并发服务，需要重新确认供应商条款，或更换适合该
  部署形态的模型服务 key。

因此，当前默认生成按钮仍保持 deterministic builder 快速路径；Hermes/MiMo 应作为
显式的 agent 深度规划/审稿路径接入。接入前先抽象 `PlannerProvider`，并通过环境变量
显式启用，避免用户误以为每次快速生成都经过了 Hermes 思考。
